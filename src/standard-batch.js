...
const CONFIG = {
    ...
    // 功能开关
    ENABLE_STANDARD_BATCH: process.env.ENABLE_STANDARD_BATCH === 'true',
    ...
    ALLOWED_TARGETS: process.env.ALLOWED_TARGETS ? process.env.ALLOWED_TARGETS.split(',').filter(addr => addr.trim() !== '') : [],
    
    // 配置文件路径
    STANDARD_BATCH_CONFIG_FILE: process.env.STANDARD_BATCH_CONFIG_FILE || 'call_data/standard-batch-config.json',
};

class EIP7702StandardBatchDemo {
    constructor(config) {
        this.config = config;
    ...
    async demonstrateStandardBatch() {
        // 打印精美的标题横幅
...
`));
        ...
        // 步骤 3: 准备标准批量交易
        console.log(chalk.magenta.bold('📌 步骤 3: 准备标准批量交易'));
        console.log(chalk.gray('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
        const standardTransactions = await this.prepareStandardBatchTransactions();
        console.log('');

        // 步骤 4: 执行标准批量交易
        console.log(chalk.magenta.bold('🚀 步骤 4: 执行标准批量交易'));
        console.log(chalk.gray('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
        const txHash = await this.executeStandardBatchTransaction(authorization, standardTransactions);
        console.log(chalk.green.bold('✅ 标准批量交易发送成功'));
...
`));
    ...
    validateConfig() {
        ...
        if (!this.config.ENABLE_STANDARD_BATCH) {
            throw new Error(chalk.red.bold('❌ 标准批量交易功能未启用，请设置 ENABLE_STANDARD_BATCH=true'));
        }
        
        // 验证日志级别
        const validLogLevels = ['error', 'warn', 'info', 'debug'];
        if (this.config.LOG_LEVEL && !validLogLevels.includes(this.config.LOG_LEVEL)) {
            console.log(chalk.yellow(`⚠️  无效的日志级别: ${this.config.LOG_LEVEL}，使用默认值: info`));
            this.config.LOG_LEVEL = 'info';
        }
        
        // 验证 Gas 价格策略
        const validGasStrategies = ['auto', 'slow', 'normal', 'fast'];
        if (this.config.GAS_PRICE_STRATEGY && !validGasStrategies.includes(this.config.GAS_PRICE_STRATEGY)) {
            console.log(chalk.yellow(`⚠️  无效的 Gas 价格策略: ${this.config.GAS_PRICE_STRATEGY}，使用默认值: auto`));
            this.config.GAS_PRICE_STRATEGY = 'auto';
        }
    }
    ...
    async prepareStandardBatchTransactions() {
        // 检查是否启用了标准批量交易
        if (!this.config.ENABLE_STANDARD_BATCH) {
            throw new Error(chalk.red.bold('❌ 未启用标准批量交易功能。请在 .env 中设置 ENABLE_STANDARD_BATCH=true'));
        }
        ...
        // 尝试从 JSON 文件加载配置
        const configFilePath = path.join(__dirname, '..', this.config.STANDARD_BATCH_CONFIG_FILE);
        ...
        try {
            ...
            standardTransactions = config.transactions.map((tx, index) => {
                // 验证必需字段
                if (!tx.target) {
                ...
                if (tx.abi && tx.params) {
                    try {
                        ...
                        console.log(chalk.green(`✅ 交易 ${index + 1}: ABI 编码成功`));
                    } catch (error) {
                        throw new Error(chalk.red.bold(`交易 ${index + 1}: ABI 编码失败 - ${error.message}`));
                    }
                } else if (tx.data) {
                    // 如果提供了直接的 data，使用它
                    encodedData = tx.data;
                ...
                return {
                    ...
                    abi: tx.abi || null,
                    params: tx.params || null
                };
            });

            console.log(chalk.green(`📁 从配置文件加载: ${this.config.STANDARD_BATCH_CONFIG_FILE}`));
            ...
            if (this.config.ENABLE_ADDRESS_WHITELIST) {
                if (this.config.ALLOWED_TARGETS.length > 0) {
                    console.log(chalk.blue(`🔒 地址白名单: ${chalk.white(this.config.ALLOWED_TARGETS.length)} 个地址`));
                } else {
                    console.log(chalk.blue(`🔒 地址白名单: ${chalk.white('已启用但未配置地址')}`));
                }
            }
            console.log('');
        ...
        // 验证交易
        this.validateStandardTransactions(standardTransactions);

        // 打印交易详情
        console.log(chalk.green.bold('\n📋 标准批量交易详情:'));
        console.log(chalk.gray('  ---------------------------------------------------------'));
        standardTransactions.forEach((tx, index) => {
            console.log(chalk.gray(`  ${index + 1}. ${tx.description}`));
            console.log(chalk.blue(`     目标地址:     ${chalk.white(tx.to)}`));
            console.log(chalk.blue(`     ETH 数量:     ${chalk.white(tx.value)} wei (${tx.value / 10n**18n} ETH)`));
            console.log(chalk.blue(`     调用数据:     ${chalk.white(tx.data)}`));
            if (tx.abi) {
                console.log(chalk.blue(`     ABI 函数:     ${chalk.white(tx.abi.name)}`));
            }
            console.log('');
        });
        console.log(chalk.gray('  ---------------------------------------------------------'));
    ...
    validateStandardTransactions(transactions) {
        let totalValue = 0n;
        ...
        for (let i = 0; i < transactions.length; i++) {
            ...
            if (!tx.to || tx.to.length !== 42 || !tx.to.startsWith('0x')) {
                throw new Error(chalk.red.bold(`交易 ${i + 1}: 无效的目标地址 ${tx.to}`));
            }
            ...
            // 验证地址白名单
            if (this.config.ENABLE_ADDRESS_WHITELIST && this.config.ALLOWED_TARGETS.length > 0 && !this.config.ALLOWED_TARGETS.includes(tx.to)) {
                throw new Error(chalk.red.bold(`交易 ${i + 1}: 目标地址 ${tx.to} 不在白名单中`));
            }
    ...
    async executeStandardBatchTransaction(authorization, standardTransactions) {
        // 编码标准批量调用数据
        const data = encodeFunctionData({
            ...
            functionName: 'executeBatchWithAuthorization',
            args: [authorization, standardTransactions]
        });

        // 计算总价值
        const totalValue = standardTransactions.reduce((sum, tx) => sum + tx.value, 0n);
...
async function main() {
    const demo = new EIP7702StandardBatchDemo(CONFIG);
    
    try {
        await demo.demonstrateStandardBatch();
    } catch (error) {
...
module.exports = { EIP7702StandardBatchDemo, CONFIG };
